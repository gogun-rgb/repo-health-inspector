...
def test_report_generation_markdown_and_json(fixture_repo_path: Path, tmp_path: Path) -> None:
    ...
    parsed = AnalysisReport.model_validate(payload)
    assert parsed.evaluated_score == 98.1
    assert parsed.skipped_rule_count == 8
    assert parsed.report_notes
    markdown = next(path for path in paths if path.suffix == ".md").read_text()
    assert "Static Analysis Disclaimer" in markdown
    assert "Raw score" in markdown
    assert "Evaluated score" in markdown
    assert "Skipped rules" in markdown
